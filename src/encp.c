...
static void derive_key(Context *ctx, char *password, size_t password_len) {
	...
	if (hydro_pwhash_deterministic(ctx->key, sizeof ctx->key, password,
			...
			PWHASH_MEMLIMIT, PWHASH_THREADS) != 0)
...
static int stream_encrypt(Context *ctx) {
	...
	while ((chunk_size = safe_read_partial(ctx->fd_in, chunk, MAX_CHUNK_SIZE)) >= 0) {
		...
		if (hydro_secretbox_encrypt(chunk, chunk, chunk_size, chunk_id, HYDRO_CONTEXT, ctx->key) != 0)
			die(0, "encryption error");
		if (write(1, chunk_size_p, 4 + hydro_secretbox_HEADERBYTES + chunk_size) < 0)
			die(1, "encryption error: write()");
		if (chunk_size == 0)
			break;
		chunk_id++;
	}
	if (chunk_size < 0)
		die(1, "encryption error: read()");
	return 0;
}
...
static int stream_decrypt(Context *ctx) {
	...
	while ((readnb = safe_read(ctx->fd_in, chunk_size_p, 4)) == 4) {
		...
		if (chunk_size > MAX_CHUNK_SIZE)
			die(0, "decryption error, chunk size too large (%zd > %zd)", chunk_size, MAX_CHUNK_SIZE);
		if (safe_read(ctx->fd_in, chunk, chunk_size + hydro_secretbox_HEADERBYTES)
				!= chunk_size + hydro_secretbox_HEADERBYTES)
			die(0, "decryption error, chunk too short (%zd bytes expected)", chunk_size);
		...
		if (chunk_size == 0)
			break;
		if (write(1, chunk, chunk_size) < 0)
			die(1, "decryption error, write()");
		chunk_id++;
	}
	if (readnb < 0)
		die(1, "decryption error, read()");
	if (chunk_size != 0)
		die(0, "decryption error, premature end of file");
	return 0;
}
...
static void options_parse(Context *ctx, int argc, char *argv[]) {
	...
	ctx->in = NULL;
	optind = 0, opterr = 0;
	while ((optflag = getopt_long(argc, argv, optstring, longopts, &longindex)) != -1) {
		switch (optflag) {
			case 'h': usage(); exit(0);
